...
def order_page(request):
    ...
    if not menus.exists():
        return render(request, "orderpage.html", {
            "menus": menus,
            "error_no_menu": "Tidak ada menu yang tersedia. Silakan tambahkan menu terlebih dahulu.",
        })
    ...
    if request.method == "POST":
        ...
        if not has_order:
            return render(request, "orderpage.html", {
                "menus": menus,
                "error_empty": "Pesanan tidak boleh kosong. Silakan pilih minimal 1 menu.",
            })
...
def antrian_page(request):
    ...
    orders_created = (
        Order.objects.filter(merchant=merchant, is_done=False)
        .prefetch_related("items")           
        .order_by("created_at")
    )

    orders_completed = (
        Order.objects.filter(merchant=merchant, is_done=True)
        .prefetch_related("items")           
        .order_by("-created_at")
    )
